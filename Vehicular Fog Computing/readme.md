# Mobile-Edge-Computing

## Vehicular Fog Computing: A Viewpoint of Vehicles as the Infrastructures

### Abstract

随着不断发展的先进车辆应用的出现，满足通信和计算需求的挑战日益突出。如果没有强大的通信和计算支持，各种车辆应用和服务仍将处于概念阶段，不能在日常生活中付诸实践。因此，解决这个问题非常重要。现有的解决方案，例如蜂窝网络，路边单元（RSU）和移动云计算，远非完美，因为它们高度依赖并承担额外基础设施部署的成本。鉴于城市地区拥有大量车辆，将这些未充分利用的车辆资源投入使用可提供巨大的机会和价值。因此，我们构想了利用车辆作为通信和计算的基础设施的想法，称为车辆雾计算（VFC），基于更好地利用每个车辆的个人通信和计算资源，该架构利用协作的多个end-user clients或near-user edge devices来执行通信和计算。通过聚合各种车辆的丰富资源，可以大大提高服务质量和应用质量。特别是，通过讨论四种类型的移动和停放车辆作为通信和计算基础设施的情景，我们对VFC的容量进行了定量分析。 我们揭示了车辆的通信能力，连通性和移动性之间的有趣关系，并且我们还发现了关于停车行为模式的特征，这得益于对利用车辆资源的理解。最后，我们讨论了实施拟议的VFC系统作为基础设施的挑战和未解决的问题。 我们的研究为这种新颖有前途的范例以及有关车辆信息基础设施的研究课题提供了见解。

### I. INTRODUCTION

城市车载网络被认为是未来智能交通系统的重要组成部分[1]，它们支持各种移动服务，从内容共享应用（如广告和娱乐）到信息传播服务（如，自然灾害和恐怖袭击的紧急行动）[2]。这些城市车辆网络通过交换有价值的信息确保了驾驶安全性，交通效率和极大的便利性。在过去十年中，随着诸如蜂窝网络和云计算等更先进的设备和技术的出现，车载网络和相关应用已经得到了显着的发展。作为这种趋势的结果，还出现了一个重要问题，即数据通信和计算能力的需求急剧增加。新的应用程序，如增强现实（AR）技术[3]，自动驾驶等，都涉及复杂的数据处理和存储操作，这需要更高水平的数据通信，计算和存储。这对现有的传统车载网络提出了巨大的挑战，特别是在通信和计算能力方面。为了满足这种对通信和计算能力不断增长的需求，使用车载云作为数据中心和增强的处理资源是一个吸引人的想法。例如，公司停车场的停放车辆云可以用于处理计算任务，或者它们可以用于彼此协作以发送消息和共享通信资源。

为了解决通信和计算容量问题，提出了一些现有方法，包括第三代（3G）和第四代（4G）蜂窝网络[4]，路边单元（RSU）[5]和移动云计算[6]。然而，由于以下原因，它们还不够。蜂窝网络提供了通信的增强但非常有限，它们主要由网络运营商控制，从应用方面来看效率和效率都不高[4]。 RSU扩大了网络通信容量，但实际上非常昂贵并且难以在道路上全面部署，特别是在大规模上，例如整个城市[5]。移动云计算可以为移动用户带来丰富的计算资源，但在上传实时信息时成本高昂且耗时，因为移动云计算使用客户端-服务器通信模型[7]。移动云计算还需要与远程基础架构的高质量网络连接[8]。因此，研究人员和工程师如何更有效，更方便地处理通信和计算需求仍然是一个巨大的挑战。

如今，全球车辆数量迅速增加，2010年达到惊人数十亿[9]。直接的结果是，交通缓慢的车辆频繁发生，停放的车辆数量非常庞大，特别是在许多城市地区。这种情况通常被认为是一场噩梦，但我们可以从一个新的角度来看待它，并将这些停放和缓慢移动的车辆视为巨大的未充分利用的资源。因此，我们最终提出了车辆雾计算（VFC）的新颖概念，其利用车辆作为基础设施来充分利用这些车辆通信和计算资源。具体而言，VFC是一种架构，它利用协作众多的end-user clients或near-user edge devices来执行大量的通信和计算[10],[11]。除了云端特性，例如向最终用户提供数据，计算，存储和应用服务外，VFC还与其他现有技术区分开来，与最终用户接近，密集的地理分布和对移动性的支持[12]。因此，VFC利用慢速行驶和停放车辆的最佳功能，例如在地点集群分布，使他们能够与附近的车辆协作。VFC修改和采用的特定雾计算多层体系结构如图1所示。以这种方式，该提出的车辆系统可以满足相当大的通信和计算能力的需求，并且与传统系统相比它工作得更好。值得强调的是，图1中描述的VFC的多层体系结构与原始雾计算范例[10]-[14]的根本不同。具体地，在雾计算架构中，车辆和移动设备是最终用户，并且在这些边缘用户和云之间存在雾服务器层。相比之下，在VFC架构中，车辆和移动设备本身也是“雾”的一部分。

![image](https://github.com/qpointwang/Mobile-Edge-Computing/blob/master/Vehicular%20Fog%20Computing/fig1.png)

将车辆视为基础设施的想法在技术上是实用的。例如，现有的工作[15]，[16]将停放的车辆视为基础设施，其发展了停放的车辆辅助的概念，以使停放的车辆能够作为静态节点加入车辆网络。无线设备和可充电电池也有助于此过程，使停放的车辆能够轻松地彼此通信并与附近的移动车辆通信。由于诸如大量，长时间停留和特定位置的特性，这些停放的车辆是用作静态主干和服务基础设施的非常理想的选择。当这些作为基础设施的车辆协同工作并形成一种新的混合网络时，城市地区的通信和计算能力都可以大大提高。

作为雾计算的扩展范例，VFC具有雾计算的许多共同特征，例如地理分布和低延迟通信。雾计算是底层网络和云之间的网络。它将传统的云计算范例扩展到网络边缘，从而可以创建更精细，更好的应用程序或服务[11]。与更加集中化的云相比，雾计算以广泛分布的部署为目标服务和应用。由于雾是本地化的，它提供低延迟通信和更多的上下文感知[17]，[20]。雾适用于低延迟通信，视频流，游戏，AR等应用。凭借雾计算的概念，研究人员已经研究了一些基于它的有用和有趣的应用。例如，Aazam等人。 [17]讨论了雾计算和基于智能网关的物联网通信架构。另一项工作[18]指出雾计算将云资源带到底层设备和物联网附近，这使其成为延迟敏感服务的理想选择。他们提出了利用雾服务进行卸载和预处理的想法，以便快速通知相关的应急处理部门。在[19]中，还提出了一个名为雾微数据中心的系统，其中雾在管理资源，执行数据过滤，预处理和安全措施方面起着至关重要的作用。

值得注意的是，VFC的关键特性与车载云计算（VCC）的关键特性不同。一些研究人员对VCC进行了一些研究并提出了自己的观点。例如，Olariu等人[21][22]已经将VCC发展成一个新的概念“自主车辆云”，其主要特征是动态汇集以服务用户并具有服务共享的自主权。他们创造了这个概念，是指一组主要是自主车，其企业计算，传感，通信和物理资源的可协调并动态分配给授权用户。 然后，他们还描述了几种应用场景，特别是停车场的数据云，商场的数据中心，以及动态同步交通灯。另外，Whaiduzzaman等人[23]也对VCC进行了调查，他们强调了这样一种观点，即对于VCC来说，街道道路和停车场上的大量车辆将被视为丰富且未充分利用的计算资源，可用于向公众提供服务。但是，我们的研究与他们的研究完全不同。虽然我们都提到了诸如使用未充分利用的基础设施等功能，但我们的VFC概念强调了新功能，而不是将信息发送到远程服务器，利用位置接近的车辆资源并让他们相互协作。 由于我们不需要向远程服务器发送信息，因此可以显着减少时间延迟并降低部署成本。地理分布，本地决策和实时负载平衡功能非常新颖，可以明确区分我们提出的概念与其他概念。

表I中比较了具体差异。一方面，VCC受带宽限制，延迟敏感，部署成本更高，而VFC基于地理分布，具有实时负载平衡和本地决策机制。更重要的是，如前所述，VFC更多地依赖于近距离车辆的协作，而不是将信息发送到远程服务器。 这大大降低了部署成本和时间延迟。另一方面，VFC可以比VCC更好，更强大地应对紧急情况。例如，如果到云或遥控器的链路断开或发生断电，VCC将遭受重创，但VFC仍可在车辆内的电源帮助下正常运行。此外，由于地理相关车辆的资源利用，与VCC相比，VFC具有更大的计算能力和更少的通信延迟。

![image](https://github.com/qpointwang/Mobile-Edge-Computing/blob/master/Vehicular%20Fog%20Computing/table1.png)

在这项工作中，我们概述了利用车辆作为基础设施的新型VFC范例的潜在能力和开放性问题。具体而言，我们考虑所有四种情景，即分别采用移动和停放的车辆作为通信和计算的基础设施。一方面，作为通信基础设施的移动和停放的车辆可以支持更大的流量，因为分组传送延迟的改进和车辆之间的增强的连接。我们可以在车辆之间采用可预测的移动模式以实现更好的通信。另一方面，作为计算基础设施的移动和停放的车辆可以最佳地利用每个车辆的计算资源。例如，拥挤的车辆可以形成车辆移动云，其由通过无线车辆到车辆通信连接的一组附近车辆组成。因此，我们提出的VFC系统可以通过聚合各个车辆的整体资源来利用巨大的计算潜力。本文的主要贡献概括如下。

* 我们提出了一种新型的VFC系统，它采用车辆作为通信和计算的基础设施，以扩大可用资源并提高可实现的容量。据我们所知，我们是第一个考虑和研究这个有趣且重要的问题的人。
* 我们分析了利用移动和停放车辆作为通信和计算基础设施的所有四种情景。 此外，我们通过真实车辆移动环境支持的特定案例研究，定量地表征通信和计算能力。
* 我们讨论了实现VFC的挑战，并指出了需要进一步研究的开放性问题。 这为该领域的未来工作提供了基本的见解和宝贵的指导。

本文的其余部分安排如下。 在第二部分中，我们描述了我们的VFC范例和关于将移动和停放的车辆用作基础设施的四种应用场景。在第三部分中，基于现实世界的车辆移动轨迹，我们提供了对所有四种应用场景的潜在能力的定量分析。第四部分指出了挑战和开放性问题，值得进一步研究。 最后，我们在第五节结束我们的工作。

### II. SYSTEM OVERVIEW AND APPLICATION SCENARIOS

图2显示了具有移动和停放车辆服务和应用的VFC的系统概述和场景。存在四种类型的场景，如下所述。前两种情况分别涉及将移动车辆用作通信和计算的基础设施。在城市地区，移动车辆不是很高速。相反，大多数车辆在进入市中心时都在缓慢移动，特别是在某些时段。我们调查了城市地区移动车辆之间的连通性，并且不出所料地发现附近的移动车辆可以很好地相互通信。 因此，城市地区的移动车辆可以用作通信基础设施。一旦它们相互连接，移动的车辆就变成非常好的计算基础设施，形成能够提供高计算能力的强大的云。另外两个场景将停放的车辆视为基础设施，以扩大通信和计算能力。城市地区的停放车辆数量巨大，还有移动车辆，代表两种重要的车辆状态。除了高峰时段，在长时间的白天和夜间，大多数车辆停放在道路或停车场。道路上的停放车辆可以在转发数据包中起到至关重要的作用，而停车场中的停放车辆可以构成一个大型的计算云。以这种方式，可以极大地增强该区域的通信和计算能力。 我们现在更详细地讨论这四种应用场景。

VANET Vehicular ad-hoc network

![image](https://github.com/qpointwang/Mobile-Edge-Computing/blob/master/Vehicular%20Fog%20Computing/fig2.png)

#### A. Moving Vehicles as Infrastructures

1）通信：车载网络通常包含两种类型的节点：车辆和RSU。 车辆通信是作为智能交通系统的一部分而开发的，该系统旨在通过整合各种信息，通过智能交通实现安全和生产力。 智能交通系统中的通信通过车辆到车辆（V2V）和车辆到基础设施（V2I）通信的组合来实现。在V2I通信中，路边基础设施无线节点（例如RSU）的能量消耗是在努力实现网络的高可靠性和长寿命时必须考虑的最重要的约束之一[24]。对于V2V通信，在车辆之间发生信息的直接交换，主要问题是两个车辆之间的距离是否小于通信范围以建立成功的连接。 可以看出，网络连接是实现信息传输的最重要问题之一，特别是对于V2V通信。

![image](https://github.com/qpointwang/Mobile-Edge-Computing/blob/master/Vehicular%20Fog%20Computing/fig3.png)

因此，我们转向使用移动车辆作为通信基础设施的VFC，以帮助解决此通信问题并带来更好的网络连接。VFC利用多车特性和车辆移动特征的优势将信息从一个地方传送到另一个地方，如图3（a）所示。移动车辆成为良好的消息载体，并且可以通过建立新连接来不断传输信息。由于VFC的期望特性，例如地理分布和本地决策，在紧密地理位置的移动车辆可以彼此协作和连接。具体而言，当一些车辆扮演通信枢纽的角色时，他们将位置接近的车辆连接在一起，从而与更多的移动接入点连接。由于这些通信集线器，形成了雾，而不是向云服务器发送信息，通过在本地利用计算和通信资源来完成任务，这涉及本地决策和地理分布特征。所有这些都带来了更少的延迟，更低的成本和更高的效率。总而言之，与VCC相比，VFC在通信方面做得更好，因为前者可以使用地理化车辆以更少的时间延迟和更低的成本部署任务，而后者需要在移动接入点和远程服务器之间进行大量信息传输。

为了表征移动车辆并将其用作基础设施，我们首先需要了解空间和时域中的车速分布。有了这些知识，我们就可以探索车载网络的连通性和移动性之间的关系。这有助于我们全面了解城市地区的通信情况，从而准确评估使用移动车辆作为基础设施的潜力。

2）计算：移动车辆，尤其是慢速行驶的车辆，是VFC计算基础设施的重要组成部分。例如，云可以由车辆形成，车辆配备有嵌入式计算机，卡在城市道路的交叉点周围，其中RSU也可以被部署以连接到远程云。这种系统的示意图在图3（b）中示出，其中RSU通过有线互联网与远程云计算中心连接。在这样的VFC系统中，借助于本地移动cloudlet，可以通过V2V通信实现卸载或加载计算。拥堵车辆还可以通过V2U与RSU通信来交换信息，而RSU通过互联网与远程云计算中心进行通信。因此，整个系统变成混合云，其聚合来自移动云和远程云的计算资源，然后将它们重新分配给车辆以满足各个车辆的计算需求。值得强调的是，移动cloudlet还可以为远程云执行计算任务。这是特别有吸引力的，因为它将由于拥塞而浪费的能量转化为有价值且有用的计算能力。

#### B. Parked Vehicles as Infrastructures

1）通信：在任何大城市地区，停放的车辆数量都很大。此外，停放的车辆广泛分布在街道停车场，室外停车场（主要是地面上的路边停车场）和内部停车场（车库或地下停车场）。与移动车辆不同，这些停放的车辆在某些长时间内具有相对不变的位置。因此，他们无法帮助将信息从一个地方直接传送到空间域。然而，作为通信基础设施的停放车辆具有其自身独特的特征。利用无线设备和可充电（车辆）电池，停放的车辆可以容易地彼此和附近的移动车辆通信。这些停放的车辆可以作为静态骨干和服务基础设施，以改善连通性。

如图4（a）所示，路边停放的车辆可以通过携带和转发包裹到其他车辆而用作通信基础设施。 中继数据包极大地改善了机会风格连接，这将显着有益于内容共享应用程序。由于具有数量大，停留时间长，地理分布广，地理位置特殊的特点，停放的车辆能够成为城市地区丰富，可靠，便捷的通信节点。这可以弥补移动车辆的缺点，其具有相对不平衡的分布和快速变化的位置。直观地，停放的车辆的数量和停车时间的分布将影响使用停放的车辆作为通信基础设施的可实现的能力和其他关键特征。

![image](https://github.com/qpointwang/Mobile-Edge-Computing/blob/master/Vehicular%20Fog%20Computing/fig4.png)

2）计算：如前所述，许多停放的车辆广泛地分布在街道停车场，室外停车场和内部停车场中。当这些停放的车辆在适当的通信条件下连接在一起时，它们可以相互协作以实现大型计算任务。因此，这些停放的车辆变得丰富的计算基础设施，提供了大量的计算资源。这种情况如图4（b）所示。根据文献，满足巨大的计算需求是具有挑战性的，因为个别车辆的计算资源有限[25]。 VFC可以很好地解决这个计算能力问题，因为VFC可以在更短的时间内以更高的效率提供强大的计算资源并实现计算任务。例如，当公司停车场中的停放车辆加入VFC时，这些车辆形成一个小型数据中心，因此可以处理需要大量计算能力的各种复杂任务。

关于使用停放的车辆作为基础设施的主题的工作非常有限。我们预计将来会提出更现实的方案和实用方法来推广这一应用。在本文中，我们评估了利用停放车辆作为基础设施的潜在计算能力，并对该应用的优势进行了初步的定量分析。

### III. CAPABILITY ANALYSIS

#### A. Data Sets and Preprocessing

为了研究城市情景中的车辆移动性和连通性，我们对两个真实的大型城市车辆轨迹进行了研究，即上海和北京。上海轨迹[26]由上海电网项目[27]收集，其中2007年2月在上海收集了4000多辆出租车的移动跟踪数据。在此跟踪中，报告通过通用分组无线服务（GPRS）发送回数据中心。具体而言，报告的频率是出租车载有乘客时每1分钟或者当空置时每15秒报告一次。报告的信息包括出租车的ID，出租车位置的经度和纬度坐标，速度以及其他因素，如航向角，以及出租车的状态。在我们的研究中，我们以下列方式预处理数据集。为了分析车辆的移动性和连通性，我们需要知道每个出租车在大量时间点的确切位置。因此，以固定频率采样适当的时间点以获得实时拓扑非常重要。由于GPS报告在15秒或1分钟的时间间隔内以离散时间并置，因此我们每10分钟采样一次数据。因此，每10分钟就可以获得一个新的实时拓扑结构，这样，我们可以在24小时内收集144个拓扑结构。在经验数据处理之后，我们发现它是可接受的采样频率，因为大多数采样拓扑在10分钟内没有显着变化。

北京的踪迹[28]是最大的城市车辆数据追踪。 在收集北京的踪迹时，我们使用了2010年5月从携带GPS接收器的27000辆北京出租车获得的移动轨道日志。具体来说，我们利用GPS设备收集出租车的位置和时间戳以及GPRS模块，每15秒报告一次移动出租车的记录。此类报告中包含的具体信息包括出租车的ID，出租车位置的经度和纬度坐标，时间戳，即时速度和航向。 类似地，在分析之前适当地预处理数据集。

#### B. Moving Vehicles

1）通信：通信容量在车载网络中具有重要意义，因为没有可靠的通信就不会发生信息交换。在影响容量的各种变量中，连接是最重要的指标之一。车辆之间的连接量从根本上表示车辆之间的网络通信条件有多好。因此，我们专注于连通性，以研究利用移动车辆作为通信基础设施的能力。由于研究车载网络的移动性如何影响其连通性（通过称为组件大小的拓扑度量衡量），我们首先通过分析上海跟踪来揭示大规模城市车辆网络的移动性和连通性之间的基本关系。

我们考虑通过定义单位元通信范围R来基于简单单位元模型建立通信链路，以判断是否建立了成功链路。在这项工作中，我们将通信范围设置为R = 600 m。为了描述大型车载网络的关键特性，我们提供以下定义。